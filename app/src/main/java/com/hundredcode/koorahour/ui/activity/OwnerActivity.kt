...
import com.hundredcode.koorahour.databinding.ActivityOwnerBinding
...
class OwnerActivity : AppCompatActivity() {
    ...
    private fun setupBottomNavigationBar() {
        binding.bottomBar.onItemSelected = { index ->
            when (index) {
                0 -> navController.navigate(R.id.owner_home_fragment)
                1 -> navController.navigate(R.id.owner_my_field_fragment)
                2 -> navController.navigate(R.id.add_field_fragment)
                3 -> navController.navigate(R.id.owner_profile_fragment)
            }
        }
    }
        ...
        onBackPressedDispatcher.addCallback(this) {
            when (navController.currentDestination?.id) {
                R.id.owner_home_fragment -> {
                    ...
                    navController.navigate(R.id.owner_home_fragment)
